...
\label{chapitre.commande}
	...
	\section{Formulation des problèmes d'optimisations}
		...
		\subsection{Lorsque les trois roues sont en contact avec le sol}
			...
			\label{section.objectifs3roues}
				...
				\liste{
					...
					\item Assurer une bonne robustesse aux perturbations. Cet objectif consiste à accroitre le potentiel d'action du robot.
				...
				Dans la suite, nous noterons $O_i$ l'objectif $i$. Il correspond à la norme élevée au carré d'une fonction linéaire des variables de commande. Il s'écrit de la façon suivante :
				\eq{
					O_i = \frac{1}{2}\norm{f_i(X)}^2
				}
		...
		\subsection{Lorsque le robot bascule sur deux roues}
			...
			\subsubsection{Formulation des contraintes}
				...
				\subsubsubsection{Limites articulaires du corps du robot}
					...
					De façon identique à la section \rf{section.ctr_body_3_roues}, on définit la contrainte entre le CoM du corps du robot $C$ et celui de la base mobile $B$ par un rectangle de dimensions $(k^x, k^y)$ centré en $b$.
		...
		\label{section.mpc_transition}
			...
			\liste{
				\item $O_3$ et $O_4$. Contrôler le CoP au milieu de la base en utilisant le modèle de robot sur trois roues permet d'assurer au robot de ne pas accroître son angle de basculement.
	...
	\section{Gestion des deux modèles dynamiques exclusifs}
		...
		\subsection{Choix d'un superviseur et conséquences}
			...
			\fig{
				\centering
				\includegraphics[width=6.5in]{tilt_process.png}
				\caption{Présentation du changement des états du superviseur au cours du temps.}
				\label{fig.tilt_process}
			}
			...
			Nous avons fait le choix d'un superviseur basé sur une machine a états et d'une heuristique de transition entre ces états \rfi{fig.tilt_process}.
		...
		\subsection{Fonctionnement du superviseur}
			...
			\subsubsection{Les transitions depuis l'état \textit{No tilt}}
				...
				\liste{
					...
					\item Dans tous les autres cas, le superviseur reste dans l'état \textit{No tilt}, car il n'est pas en train de basculer.
				}
	...
	\section{Conclusion}
		...
		\liste{
			\item Lorsque le robot est en contact sur le sol avec ses trois roues.
